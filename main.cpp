...
int main()
{
	string filename = "D:\\University\\Data Structures and Algorithms\\Mini Excel\\Mini Excel\\Debug\\data.txt";
	Excel excel;
	Excel::Cell* start = nullptr;
	Excel::Cell* end = nullptr;
	vector<string> clipboard;
	FrontEnd fe(excel.root);
	excel.loadSheet(filename);
	fe.displayGrid(excel.current);
	fe.showCurrentCell(excel.current, fe.x, fe.y);
	while (true)
	{
		if (GetAsyncKeyState(VK_LEFT))
		{
			if (excel.current->left != nullptr)
			{
				fe.printCell(excel.current, fe.x, fe.y);
		...
		{
			if (excel.current->right != nullptr)
			{
				fe.printCell(excel.current, fe.x, fe.y);
		...
		{
			if (excel.current != nullptr)
			{
				excel.current->data = fe.input();
				fe.printDataInCell(excel.current);
			}
		}
		...
		else if (GetAsyncKeyState(VK_INSERT))
		{
			if (GetAsyncKeyState('C'))
			{
				...
				{
					clipboard = excel.copy(start, end);
				}
			}

			else if (GetAsyncKeyState('V'))
			{
				if (!clipboard.empty() || start != nullptr || end != nullptr)
				{
					bool flag = excel.isColumn(start, end);
					excel.paste(clipboard, flag);
					system("cls");
					fe.displayGrid(excel.root);
				}
			}
			...
			else if (GetAsyncKeyState('O'))
			{
				if (start != nullptr || end != nullptr)
				{
					double sum = excel.getRangeSumResult(start, end);
					double avg = excel.getRangeAverageResult(start, end);
					fe.printSumAvg(excel.root, sum, avg);
				}
			}
			...
			else if (GetAsyncKeyState('W'))
			{
				if (start != nullptr || end != nullptr)
				{
					excel.getRangeAverage(start, end);
					fe.printDataInCell(excel.current);
				}
			}
			...
			else if (GetAsyncKeyState('R'))
			{
				if (start != nullptr || end != nullptr)
				{
					excel.getRangeMinimum(start, end);
					fe.printDataInCell(excel.current);
				}
			}
			...
			else if (GetAsyncKeyState('T'))
			{
				if (start != nullptr || end != nullptr)
				{
					excel.getRangeMaximum(start, end);
					fe.printDataInCell(excel.current);
				}
			}
		}
